...
def main() -> None:
    manifest = generate_manifest(Path("release"))

    for server in SERVER_FILES:
        inject_manifest(Path("release")/server, manifest)


def inject_manifest(zip_path: Path, manifest: str) -> None:
    with ZipFile(zip_path, "a", compression=ZIP_DEFLATED) as z:
        z.writestr("build.json", manifest)


def generate_manifest(dir: Path) -> str:
    ...
    for platform, filename in FILES.items():
        ...
        hashes[platform] = sha256_file(dir/filename)
